...
La classe 'H2Matrix' contient déjà un produit scalaire matrice-vecteur nommé $.dot()$. Il est alors intéressant d'étudier l'erreur commis par cette opération pour une matrice aléatoire de taille $N \times N$, et en# Mise en place d'un solveur pour matrice $\mathcal{H} ^ 2$
...
Il est peut-être envisageable de coder un solveur itératif de Krylov nous-mêmes ! fonction de $\tau > 0$
