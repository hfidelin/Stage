...
!-----------------------------------------------------------------------
      subroutine iluk(n,nnz,nnzo,a,ja,ia,lfil,alu,jlu,ju,levs,iwk,ierr)
      implicit none 
      integer, intent(in) :: n,nnz,nnzo, lfil,ja(nnz),ia(n+1)
      integer, intent(out) :: ierr,ju(n)
      real(8), intent(in) :: a(nnz)
      real(8) :: w(n)
      integer:: jw(3 * n)
      real(8), intent(out) :: alu(nnzo)
      integer, intent(out) :: jlu(nnzo), levs(iwk)
...
!-----------------------------------------------------------------------
       ...
       subroutine ig (n, nnz, a, ia, ja, b, iwk, x, im,ierr1,ierr2,alu,jlu,ju)
       integer, intent(in) :: n,nnz,ja(nnz),ia(n+1),iwk
       real(8), intent(in) :: a(nnz),b(n)
       real(8), intent(out) ::alu(iwk), x(n)
        integer, intent(out) :: jlu(iwk), ju(n+1), ierr1,ierr2
        
       integer  jw(2*n), lfil, im,maxits,iout
       real*8  w(n+1), dt, sol(n),vv(n*(im+1)),eps
       
       write(*,*) "ILUT GMRES!"
       lfil = 2
       dt = 1e-3
       call ilut(n,a,ja,ia,lfil,dt,alu,jlu,ju,iwk,w,jw,ierr1)
       sol = b
!       vv = 0
       eps = 1e-3
       maxits = 9999
       iout = 1
       call pgmres(n, im, b, sol, vv, eps, maxits, iout, a, ja, ia, alu, jlu, ju, ierr2)
       x = sol
       end
